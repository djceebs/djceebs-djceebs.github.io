...
bowser_palette <- colorNumeric(palette = "Greens", 
                              domain = bowser_df$bowser_marginpct)
twhite_palette <- colorNumeric(palette = "Oranges", 
                               domain = twhite_df$twhite_marginpct)
rwhite_palette <- colorNumeric(palette = "Blues", 
                               domain = rwhite_df$rwhite_marginpct)
...
leaflet() %>%
  ...
  addPolygons(
    data = bowser_df,
    fillColor = ~bowser_palette(bowser_df$bowser_marginpct),
    stroke = TRUE,
    smoothFactor = 0.2,
    fillOpacity = 0.8,
    color = "#666",
    ...
    popup = paste0("Winner: Muriel Bowser", "<br>",
      ...
      "Margin of Victory: ", 
      round(bowser_df$bowser_marginpct *100), "%") 
  ) %>%
  addPolygons(
    data = twhite_df,
    fillColor = ~twhite_palette(twhite_df$twhite_marginpct),
    stroke = TRUE,
    smoothFactor = 0.2,
    fillOpacity = 0.8,
    color = "#666",
...
addPolygons(
  data = rwhite_df,
  fillColor = ~rwhite_palette(rwhite_df$rwhite_marginpct),
  stroke = TRUE,
  smoothFactor = 0.2,
  fillOpacity = 0.8,
  color = "#666",
